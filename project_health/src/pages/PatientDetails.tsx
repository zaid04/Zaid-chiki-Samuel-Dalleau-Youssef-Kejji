...
export default function PatientDetails() {
  ...
  useEffect(() => {
    Promise.all([
      api.get(`/items/people/${id}`),                         // ⬅️  nouveau !
      api.get('/items/physiologicalData', { params: { 'filter[people_id]': id } }),
      api.get('/items/physicalActivities', { params: { 'filter[people_id]': id } }),
    ])
      .then(([p, phys, act]) => {
        setPerson(p.data.data);
        setPhysio(phys.data.data);
  ...
  return (
    <div className="mx-auto max-w-3xl p-4">
      <Link to="/patients" className="mb-4 inline-block text-blue-600 hover:underline">
        ← Retour à la liste
      </Link>

      <h1 className="mb-6 text-2xl font-semibold">
        Fiche patient {person ? `${person.firstname} ${person.lastname}` : `#${id}`}
      </h1>
